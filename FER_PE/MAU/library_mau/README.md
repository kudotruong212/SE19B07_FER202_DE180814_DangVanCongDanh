# Library Management System

Hệ thống quản lý thư viện sách được xây dựng với React, sử dụng Context API và useReducer để quản lý state, React Router để điều hướng, và JSON Server làm backend API.

## 🚀 Tính năng

- **Đăng nhập/Đăng xuất**: Xác thực người dùng với role admin
- **Quản lý sách**: 
  - Xem danh sách sách
  - Thêm sách mới
  - Chỉnh sửa thông tin sách
  - Xóa sách
  - Xem chi tiết sách
- **Quản lý người dùng**:
  - Xem danh sách người dùng
  - Xem chi tiết người dùng
  - Khóa/Mở khóa tài khoản
  - Tìm kiếm và lọc theo role, status
- **Tìm kiếm & Lọc**:
  - Tìm kiếm theo title, author, ISBN (sách)
  - Tìm kiếm theo username, full name (người dùng)
  - Lọc theo category và status (sách)
  - Lọc theo role và status (người dùng)
  - Sắp xếp theo nhiều tiêu chí
- **Thống kê**: Hiển thị tổng số sách và tổng giá trị

## 📋 Yêu cầu

- Node.js (v14 trở lên)
- npm hoặc yarn

## 🔧 Cài đặt

1. Cài đặt dependencies:
```bash
npm install
```

2. Khởi chạy JSON Server (API):
```bash
npm run api
```
Server sẽ chạy tại http://localhost:3001

3. Khởi chạy ứng dụng React (trong terminal khác):
```bash
npm start
```
Ứng dụng sẽ chạy tại http://localhost:3000

## 📁 Cấu trúc dự án

```
src/
├── components/          # Các components tái sử dụng
│   ├── AddBookForm.jsx
│   ├── BookTable.jsx
│   ├── ConfirmModal.jsx
│   ├── EditBookModal.jsx
│   ├── FilterBar.jsx
│   ├── LoginForm.jsx
│   ├── NavigationHeader.jsx
│   ├── UserFilter.jsx
│   ├── UserTable.jsx
│   ├── ViewBookDetailsModal.jsx
│   └── ViewUserDetailsModal.jsx
├── contexts/           # Context API để quản lý state
│   ├── AuthContext.jsx
│   └── BookContext.jsx
├── pages/              # Các trang chính
│   ├── AddBookPage.jsx
│   ├── DashboardPage.jsx
│   ├── LoginPage.jsx
│   └── UserListPage.jsx
├── routes/             # Định nghĩa routes
│   └── AppRoutes.js
├── services/           # API services
│   └── api.js
├── App.js
└── index.js
```

## 🔐 Thông tin đăng nhập

Mặc định có 2 tài khoản admin:

1. **Username**: `admin` / **Password**: `123456`
2. **Username**: `librarian` / **Password**: `123456`

## 📚 Cấu trúc dữ liệu Book

Mỗi cuốn sách có các trường:
- `id`: ID duy nhất
- `title`: Tên sách
- `author`: Tác giả
- `isbn`: Mã ISBN
- `category`: Thể loại (Programming, Web Development, Database, Computer Science, ...)
- `price`: Giá (VND)
- `publishedDate`: Ngày xuất bản
- `description`: Mô tả
- `status`: Trạng thái (available, borrowed, reserved)
- `imageUrl`: URL hình ảnh (optional)

## 🛠️ Công nghệ sử dụng

- **React**: UI framework
- **React Router**: Điều hướng
- **Context API + useReducer**: State management
- **React Bootstrap**: UI components
- **Axios**: HTTP client
- **JSON Server**: Mock API server

## 📝 Scripts

- `npm start`: Khởi chạy ứng dụng React (port 3000)
- `npm run api`: Khởi chạy JSON Server (port 3001)
- `npm run build`: Build ứng dụng cho production
- `npm test`: Chạy tests

## 🎯 Cách sử dụng

### Quản lý Sách:
1. Đăng nhập với tài khoản admin
2. Trang Dashboard hiển thị danh sách sách với bộ lọc và tìm kiếm
3. Click "Thêm Sách" để thêm sách mới
4. Click "View" để xem chi tiết sách
5. Click "Edit" để chỉnh sửa sách
6. Click "Delete" để xóa sách

### Quản lý Người dùng:
1. Click "User Management" trên thanh điều hướng
2. Xem danh sách tất cả người dùng
3. Click "View Details" để xem chi tiết người dùng
4. Click "Ban Account" để khóa tài khoản
5. Click "Unban Account" để mở khóa tài khoản
6. Sử dụng bộ lọc để tìm kiếm và lọc theo role, status

## 📖 Học từ dự án này

Dự án này minh họa các khái niệm:

1. **Context API**: Quản lý state toàn cục (AuthContext, BookContext)
2. **useReducer**: Quản lý state phức tạp với actions và reducers
3. **React Router**: Điều hướng và bảo vệ routes (PrivateRoute)
4. **API Integration**: Tích hợp với REST API qua Axios
5. **Form Handling**: Xử lý form với validation
6. **Component Composition**: Tái sử dụng components
7. **State Management**: Quản lý state với filters, sorting, pagination

## 🔄 So sánh với progress-test2

Dự án này được chuyển đổi từ hệ thống quản lý thanh toán (payment-user) sang hệ thống quản lý sách (books):

- **Payment → Book**: Thay đổi entity chính
- **PaymentContext → BookContext**: Context mới với logic phù hợp
- **PaymentTable → BookTable**: Table component mới
- **Fields thay đổi**: 
  - Payment: semester, courseName, amount, date
  - Book: title, author, isbn, category, price, publishedDate, description, status

## 📄 License

MIT

## 👨‍💻 Author

Dự án được tạo dựa trên mẫu progress-test2, chuyển đổi sang hệ thống quản lý thư viện sách.